...
\todo[inline]{not the subscription operation itself must be modeled, only the meta information. ==> Shortcoming2}
...
\todo[inline]{show what that would look like in example 2.}
...
\todo[inline]{do i keep this chapter?}
...
After the previous section has described the functionality and usage of the BPMN extension, the following section defines the additions necessary to the BPMN execution semantics.
...
For this purpose, the activity live cycle is adopted to event elements as follows:
Once an event is enabled, it immediately enters the state \textit{running}. 
Event elements can be canceled when they appear after an event-based gateway and another event following the gateway occurs earlier.
...
\paragraph{Operational semantics of subscription handling}



- general: we specify the time the following operations must be executed. register query (includes inst. of buffer), subscribe (to buffer), unsubscribe, remove query. on the basis of <subscriptionDefinition>
...
\todo[inline]{write exec semantics}
