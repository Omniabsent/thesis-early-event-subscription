...
Notably, the Camunda process model appears less complex than the one presented in \autoref{ass:model:buffered} while still implementing the same functionality. That is possible because in Camunda, the information received in intermediate message events is written to process variables for further use, a feature that the BPMN specification only considers for the receive task.
