...
\todo[inline]{after writing this, compare with table}


\section{BPMN Models in presence of the Event Occurrence Scenarios}\label{ch:ass:models}
\todo[inline]{To keep the illustrations simple, implicit subscription and unsubscription as part of the event-element is assumed}
...
\end{figure}
\todo[inline]{watch that the full-page figure is correctly hspaced (left/right)}
...
\section{Discussion and Requirements Extension}\label{ch:ass:discussion}
...
However, only two of them are considered to be natively supported by the standard while most of the solutions reveal certain shortcomings when it comes to modeling flexible event subscription using BPMN. 
%requirements add additional complexity to the solution. %specific work-arounds had to be identified.
In the following, the identified shortcomings are discussed one after the other, which leads to the definition of additional requirements.
...
Another dimension becomes apparent in the BPMN model for buffered event handling that is presented in \autoref{ass:model:buffered}. While it allows the subscription to events even before process deployment, it requires the execution of an additional set of auxiliary processes for that purpose.
...
\subsection*{Shortcoming S3: Event buffering and subscription are infrastructure tasks.}

By their nature 

- more instances and processes to maintain and manage using the user interfaces
- the it department has to do it
- it has to be explicitly modeled
- it might be too complicated for process designers to model
...
%\paragraph{Buffering is an infrastructure task\newline}
...
process designers are not it guys


\subsection*{Shortcoming S2: Implementing event buffering in the process engine is likely to cause performance issues}
...
An aspect that cannot be influenced without tempering with the process engine code.
...
\subsection*{Requirements extension}\label{ch:ass:reqextension}
