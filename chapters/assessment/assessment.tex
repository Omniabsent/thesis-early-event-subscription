...
\subsection*{Shortcoming S2: Event buffering and subscription are infrastructure tasks.}
...
The previously discussed shortcomings S1 to S3 show that even though the basic functional requirements \textit{R1} to \textit{R3} are fulfilled, the resulting solution is not acceptable for application in a production environment.
Inspired by the highlighted issues, three additional requirements are formulated in the following. While \textit{R4} is an additional functional requirement, \textit{NFR1} and \textit{NFR2} are considered non-functional requirements~(NFRs).


\begin{description}
	\item[R4 Automatic Subscription Handling:] 
...
\begin{description}
	\item[NFR1 Minimal increase in Model Complexity:] 	
\end{description}
