...
\begin{figure}[]
	...
	\caption{Abstract Process using an intermediate message catch event}\label{fig:abstract-process-with-event}
\end{figure}
...
\paragraph{EOS2: The event does not occur}
\begin{figure}[]
	\myfloatalign
	{\includegraphics[width=1\linewidth]{chapters/assessment/parallel-timer-event.png}}
	\caption{Intermediate message event accompanied by a parallel timer event}\label{fig:excerpt-event-with-timer}
\end{figure}
\todo[inline]{alternatively: a receive task with boundary timer event. when gateway: don't join with exclusive}
...
\autoref{fig:excerpt-event-with-timer} shows how this behavior can be implemented by the help of an event-based gateway which puts a timer event in parallel to the intermediate catch event. If the timer event occurs before the message event is consumed, the message event is terminated immediately and the process flow proceeds from the timer event.
An alternative solution with equivalent semantics is depicted in \autoref{}, using a receive task and an interrupting boundary timer event. Once the timer event fires, the receive activity is canceled and the process continues along the outgoing flow of the time event.
...
\paragraph{EOS3: Occurrence between Process instantiation and the enabling of the BPMN event}
\begin{figure}[]
	\myfloatalign
...
\paragraph{EOS4 and EOS5: Before Process Instantiation}
\begin{figure}[]
	\myfloatalign
